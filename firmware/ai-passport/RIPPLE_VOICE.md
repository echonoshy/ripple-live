...
- 单击 `UP` / `DOWN` 以 10% 为步进调节音量，设置掉电保留。
- 长按 `DOWN` 临时显示三秒电量、Wi-Fi 信号和后端状态。
- 长按 `UP` 三秒，显示提示后清除网络配置并重新进入配网。
...
Wi-Fi、Gateway 地址和音量保存在 NVS。要更换网络，长按 `UP` 三秒清除网络
配置；音量使用独立命名空间，不会随重新配网被重置。
...
- 单击 `UP` / `DOWN` 时音量即时变化并显示临时提示，下一轮播放不会恢复默认值。
- 长按 `DOWN` 显示的电量、RSSI 和 AI 连接状态与串口日志一致。
- 长按 `UP` 不会立即误触；满三秒后显示清除提示并进入配网。
