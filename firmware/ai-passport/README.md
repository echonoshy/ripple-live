...
> This checkout contains the **Ripple Voice Passport** product firmware. It keeps
> one interaction only: hold `OK` to talk, release it to send, and listen to the
> streamed reply. See [`RIPPLE_VOICE.md`](RIPPLE_VOICE.md) for setup, architecture,
> build, flashing, and acceptance details. The rest of this README documents the
> upstream FoloToy hardware baseline retained in this directory.
